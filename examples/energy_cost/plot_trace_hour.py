...
def seconds_to_hhmm(x, pos=None):
    """Convierte segundos (instante 0 = 00:00) a etiqueta HH:MM.
    No da la vuelta a las 24h; si la traza dura más de un día,
    seguirá contando (25:30, 26:00, ...)."""
...
def plot_trace(jobs, output_path):
    ...
    # Rango temporal global, para elegir un intervalo de ticks razonable
    max_time = jobs['finish_time'].max()
    ...
    for idx, run in enumerate(runs):
        ...
        ax.set_xlabel("Time (HH:MM)", color="#aaaacc", fontsize=8)
        ax.set_ylabel("Job ID", color="#aaaacc", fontsize=8)

        # --- Formato de eje X en HH:MM ---
...
def choose_tick_step(max_time_seconds):
    """Elige un intervalo de ticks (en segundos) que dé lugar a marcas
    HH:MM legibles (~8-12 ticks en el eje), redondeado a minutos habituales."""
    if max_time_seconds <= 0:
        return 60
    target_ticks = 10
    raw_step = max_time_seconds / target_ticks
    # Intervalos "bonitos" en minutos
    nice_minutes = [1, 2, 5, 10, 15, 30, 60, 120, 180, 360, 720, 1440]
    raw_minutes = raw_step / 60
    for m in nice_minutes:
        if m >= raw_minutes:
            return m * 60
