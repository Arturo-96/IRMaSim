...
class Job:
    ...
    def __init__(self, id: int, name: str, submit_time: float, nodes: int, ntasks: int, ntasks_per_node: int, req_ops : int, ipc : float, req_time : float, req_energy : int, mem : int, mem_vol : float, slack: float):
        self.tasks = None
        self.id = id
        self.rand = 0
        self.name = str(name)
    ...
    def from_profile(klass, id: int, name: str, submit_time: float, nodes: int, ntasks: int, ntasks_per_node: int, profile: dict, type: str):
        ...
        self=klass(id,name,submit_time, nodes, ntasks, ntasks_per_node, profile['req_ops'], ipc,
                   profile['req_time'], req_energy, mem, mem_vol, slack)
        self.type = type
    ...
    @classmethod
    def header(klass):
        return "id,req_time,ntasks,mem,submit_time,start_time,finish_time,execution_time,operations,mem_vol,profile,resources,slack"
